...
const ordinalLessThanThirteen = new Map<string, string>([
    ["zero", "zeroth"],
    ["one", "first"],
    ["two", "second"],
    ["three", "third"],
...
export function makeOrdinal(words: string) {
    ...
    // Ends with one through twelve
        ...
        return words.replace(ENDS_WITH_ZERO_THROUGH_TWELVE_PATTERN, replaceWithOrdinalVariant(words));
    }
    return words;
}

function replaceWithOrdinalVariant(numberWord: string) {
    return ordinalLessThanThirteen.get(numberWord) || '';
}